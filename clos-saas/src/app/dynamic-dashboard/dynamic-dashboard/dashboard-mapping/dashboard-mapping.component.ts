...
import { CustomiseDashboard,GridStackElement, SearchScope, Widgets } from '../models/model';
...
@Component({
  selector: 'app-dashboard-mapping',
  templateUrl: './dashboard-mapping.component.html',
  styleUrls: ['./dashboard-mapping.component.scss']
})
export class DashboardMappingComponent implements OnInit {
  ...
  constructor(
    ...
    if(!this.customLayout.id){
      ...
      this.customLayout.userName = this.jwtAuthenicationService.getAuthenticatedUser();
    }else{
  ...
  async ngOnInit() {
    ...
    this.screenName=sessionStorage.getItem('dashName')
    ...
    // this.showQueryData(this.queryText,this.index);
  }
  removeDashName(){
    sessionStorage.removeItem('dashName')
  }
...
openErrSnackbar(message){
  ...
  clearFilter(i){
    ...
    this.customService.getQueryData(this.customLayout.widget[i].type,this.customLayout.widget[i].queryText,this.loadMoreGraphs,this.searchScopeOfGraph.pageSize).subscribe(
      ...
      if(this.customLayout.widget[i].type=='candleStick'){
        this.customService.getQueryData(this.customLayout?.widget[i]?.type,this.customLayout.widget[i].queryText,this.loadMoreCandle,this.searchScopeOfGraph.pageSize).subscribe(
          res=>{
            let data:any[]=[];
              data=res['data'];
              this.funnelCount=res['count']
