...
import { RolloverApplicationPopupComponent } from '../rollover-application-popup/rollover-application-popup.component';

@Component({
  selector: 'app-rollover-application-listing',
  templateUrl: './rollover-application-listing.component.html',
  styleUrls: ['./rollover-application-listing.component.scss']
})
export class RolloverApplicationListingComponent implements OnInit {

  activeTabIndex: number = 0;
  matTabHeader: any[] = [
    { name: 'Roll Over Applications', icon: 'list' },
    // { name: 'Draft Application', icon: 'save' },
    ]
  selectedTabName: string = 'allApplications';
  selectedNavigation: string = 'ROLL_OVER';
  fieldsName: any[];
  displayName: any[];
  datefieldsName: any[];
  ...
  constructor(
    ...
    if (this.activeTabIndex == 0) {
      ...
      this.selectedNavigation = 'ROLL_OVER';
    }
  ...
  getExtraColumns() {
    if (this.selectedNavigation == 'ROLL_OVER') {
      this.extraColumns = [
        {
          fieldName: "$rollover_view_details",
        ...
        {
          fieldName: "$rollover_document_preview",
        ...
        {
          fieldName: "$rollover_parent_details",
        ...
        // {
        //   fieldName: "$rollover_details",
        //   displayName: "Rollover Details",
        //   lockColumn: false,
        //   columnDisable: true,
        //   searchText: [],
        //   sortAsc: null,
        ...
        // },
        {
          fieldName: "rollover_details_view",
          displayName: "View Rollover Details",
          lockColumn: false,
          columnDisable: true,
          searchText: [],
          sortAsc: null,
  ...
  detailClick(clickEvent: ClickEvent) {
    ...
    //Rollover button Click
    // if (clickEvent.col && clickEvent.col.displayName === 'Rollover Details') {
    //   let fsIds = clickEvent.data.id;
    //   let buttonName = clickEvent.col.displayName;
    //   const dialogRef = this.dialog.open(RolloverApplicationPopupComponent, {
    //     width: '130rem', height: '80rem', data:{ fsIds: clickEvent.data.id, buttonName: clickEvent.col.displayName}
    //   });
    // }
    // View Rollover Click
    if (clickEvent.col && clickEvent.col.displayName === 'View Rollover Details') {
      let fsIds = clickEvent.data.id;
      let buttonName = clickEvent.col.displayName;
      const dialogRef = this.dialog.open(RolloverApplicationPopupComponent, {
        height: '80rem',
        width: '130rem', data:{ fsIds: clickEvent.data.id, buttonName: clickEvent.col.displayName}
      });
    }
