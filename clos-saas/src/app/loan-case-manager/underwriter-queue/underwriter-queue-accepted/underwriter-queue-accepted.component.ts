...
import { AcceptedDetailComponent } from './accepted-detail/accepted-detail.component';
import { PageEvent } from '@angular/material/paginator';

@Component({
  selector: 'app-underwriter-queue-accepted',
  templateUrl: './underwriter-queue-accepted.component.html',
  styleUrls: ['./underwriter-queue-accepted.component.scss']
})
export class UnderwriterQueueAcceptedComponent implements OnInit {

  pageNum: number = 0;
  pageSize: number = 15;
  verificationStatus: string = 'ACCEPTED';
  ...
  queue = ['Id','Company Name','Company Type','Applicant Name','Contact Number','Loan Type','SEC/DTI Registration No','Loan Amount','Loan Purpose','Verification Status','Assigned To', 'Edit'];
...
openEditDialog(id): void {
  const dialogRef = this.dialog.open(AcceptedDetailComponent, {  
    data:id
  });
}
