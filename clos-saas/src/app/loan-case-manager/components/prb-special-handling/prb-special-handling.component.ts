...
@Component({
  selector: 'app-prb-special-handling',
  templateUrl: './prb-special-handling.component.html',
  styleUrls: ['./prb-special-handling.component.scss']
})
export class PRBSpecialHandlingComponent implements OnInit {

  selectedNavigation: string ='PRB_DUPLICATE_VERIFICATION';
  ...
  detailClick(event: any) {
    if (event.col && event.col.displayName === 'View Details') {
      ...
      let fsId = event.data.id
      let url = this.currentUrl.substr(0, this.currentUrl.lastIndexOf("/ntp-approve-list"));
      url = url + '/' + fsId;
  ...
  downloadFile(res, format: string) {
    ...
    anchor.download = `PRB-Special-Handling${latest_date}.${format}`;
    anchor.href = url;
    anchor.click();
  }
