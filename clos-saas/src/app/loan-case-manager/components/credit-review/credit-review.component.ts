...
import { BureauScreenComponent } from './modals/bureau-screen/bureau-screen.component'
@Component({
...
export class CreditReviewComponent implements OnInit {
  ...
  selectedNavigation: string ='DUP_DEPOSITOR_BASE_VERIFICATION';
  ...
  mainTabs = [
    { name: 'Verification of Changes' , icon:'remove_red_eye'},
    { name: 'Approve/Reject',icon:'assignment_turned_in' }
  ];
  ...
  subTabs = [
    [
      ...
      { name: 'Verification In Progress', icon: 'refresh' },
      { name: 'Approve', icon: 'done_all' },
      { name: 'Refer to fraud', icon: 'person' },
      { name: 'Cancel', icon: 'close' },
      { name: 'Decline', icon: 'close' },
    ],
    [
      { name: 'Verification Queue', icon: 'list' },
      { name: 'Verification In Progress', icon: 'refresh' },
      { name: 'Approve', icon: 'done_all' },
      { name: 'Reject', icon: 'close' },
    ] // No sub tabs for the "Approve/Reject" main tab
  ];
  ...
  onMainTabChangeEvent(event: MatTabChangeEvent): void {
    console.log('Main Tab Changed:', event.index);
  }

  onSubTabChangeEvent(event: MatTabChangeEvent, mainTabIndex: number): void {
    console.log('Sub Tab Changed in Main Tab', mainTabIndex, ':', event.index);
  ...
  getExtraColumns() {
    if (this.selectedTabName === 'verificationQueue') {
      ...
      this.extraColumns = [
        ...
        {
          fieldName: "$view_details_depositeer_other",
          displayName: "View Details",
          lockColumn: false,
          columnDisable: true,
          searchText: [],
...
// TO VERIFY POP UP
    if (event.col && event.col.displayName === 'Final Decision') {
      console.log("event click", event.data.id)
      const dialogRef = this.dialog.open(ToVerifypopupComponent, {
            width: '1400px', height: '850px', data: event.data.id
          });
    }
  ...
  downloadFile(res, format: string) {
    ...
    anchor.download = `Credit-review${latest_date}.${format}`;
    anchor.href = url;
    anchor.click();
  }
  ...
  //Bureau screen popup
  openBureau(){
    console.log('bureau screen popup')
    const dialogRef = this.dialog.open(BureauScreenComponent, {
      width: '90vw',
      height: '80vh',
      data: {}
    });
