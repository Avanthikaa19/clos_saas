...
@Component({
  selector: 'app-ntb-approve-stage',
  templateUrl: './ntb-approve-stage.component.html',
  styleUrls: ['./ntb-approve-stage.component.scss']
})
export class NTBApproveStageComponent implements OnInit {

  selectedNavigation: string ='NTB_T24_VERIFICATION';
  ...
  detailClick(event: any) {
    ...
    if (event.col && event.col.fieldName === '$duplicate_preview') {
      ...
      const dialogRef = this.dialog.open(DuplicatePreviewPopupComponent, {
        height: '80rem',
        width: '130rem',
