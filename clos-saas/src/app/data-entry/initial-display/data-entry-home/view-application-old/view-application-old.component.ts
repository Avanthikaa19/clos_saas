...
@Component({
  selector: 'app-view-application-old',
  templateUrl: './view-application-old.component.html',
  styleUrls: ['./view-application-old.component.scss']
})
export class ViewApplicationOldComponent implements OnInit {
  ...
  getDateFieldsName() {
    this.defaultService.getDataentrydateFieldsName().subscribe(res => {
      this.datefieldsName = res;
    })
  }
  // GET  TABLE FIELDS 
  getTableFieldsName() {
  ...
  dynamicColumndefinitions() {
    this.fieldsName?.forEach(value => {
      if (this.datefieldsName?.includes(value)) {
        let columnDef: ColumnDefinition = new ColumnDefinition();
        columnDef.fieldName = value;
        columnDef.displayName = value.replace(/([A-Z])/g, ' $1').trim();;
