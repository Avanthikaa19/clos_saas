...
@Component({
  selector: 'app-data-injector-trades-configuration',
  templateUrl: './data-injector-trades-configuration.component.html',
  styleUrls: ['./data-injector-trades-configuration.component.scss']
})
export class DataInjectorTradesConfigurationComponent implements OnInit {
  options:any = {maxLines: 5000, printMargin: false, useWorker: false};
  //two way binding to parent component
  configValue: DataInjectorTradesConfigModel;
  ...
  ngOnInit(): void {
    if (this.config == undefined || this.config == null) {
      this.flowManagerDataService.getDefaultConfiguration('DATA_INJECTOR_TRADES').subscribe(
        res => {
          this.config = res;
          //for initializing new var and minimizing older ui/api impact
...
export class DataInjectorTradesConfigModel {
  task: {
