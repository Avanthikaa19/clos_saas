...
import { ExtractionQueryDetailComponent } from '../modals/extraction-query-detail/extraction-query-detail.component';

@Component({
  selector: 'app-extraction-queries-list',
  templateUrl: './extraction-queries-list.component.html',
  styleUrls: ['./extraction-queries-list.component.scss'],
  animations: [ fadeInOut ]
})
export class ExtractionQueriesListComponent implements OnInit, ReportPortalDeveloperToolsFeature {
  ...
  refreshItems() {
    ...
    this.reportPortalDataService.getAllExtractionsByPageAndName(this.pageSize, this.currentPage-1, this.nameSearch ? this.nameSearch : ' ').subscribe(
      res => {
        ...
        this.showNotification('default', this.extractions.length ? 'Loaded ' + this.extractions.length + ' extraction queries.' : 'No extraction queries found.');
        // TESTING
  ...
  openItemDetails(extraction: ExtractionQuery) {
    const dialogRef = this.dialog.open(ExtractionQueryDetailComponent, {
      width: '1300px',
